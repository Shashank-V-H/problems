...
/* Alternate method, not as time efficient as above solution
    ...
    bool isValidSudoku(vector<vector<char>>& board) {
        ...
        for (int i = 0; i < 9; i++) {
            for (int j = 0; j < 9; j++) {
                ...
                if (rows[i].count(num) || cols[j].count(num) || subgrids[(i / 3)
* 3 + (j / 3)].count(num)) { return false;
                }

                rows[i].insert(num);
                cols[j].insert(num);
                subgrids[(i / 3) * 3 + (j / 3)].insert(num);
            }
        }
